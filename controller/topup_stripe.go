...
func (*StripeAdaptor) RequestAmount(c *gin.Context, req *StripePayRequest) {
	...
	if req.Amount > 10000 {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "top-up amount cannot be greater than 10000"})
		return
	}
	id := c.GetInt("id")
...
func StripeWebhook(c *gin.Context) {
	...
	callerIp := c.ClientIP()
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook signature verification succeeded event_type=%s client_ip=%s path=%q", string(event.Type), callerIp, c.Request.RequestURI))
	switch event.Type {
...
func sessionCompleted(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if "complete" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.completed status abnormal, ignored trade_no=%s status=%s client_ip=%s", referenceId, status, callerIp))
		return
	}
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failed event missing order id client_ip=%s", callerIp))
		return
	}
	...
	if topUp.PaymentProvider != model.PaymentProviderStripe {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe async payment failed but order payment provider mismatch trade_no=%s payment_provider=%s client_ip=%s", referenceId, topUp.PaymentProvider, callerIp))
		return
	}

	if topUp.Status != common.TopUpStatusPending {
		logger.LogInfo(ctx, fmt.Sprintf("Stripe async payment failed but order status is not pending, ignored trade_no=%s status=%s client_ip=%s", referenceId, topUp.Status, callerIp))
		return
	}
...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) {
	if len(referenceId) == 0 {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe missing order id when completing order client_ip=%s", callerIp))
		return
	}
...
func sessionExpired(ctx context.Context, event stripe.Event) {
	...
	status := event.GetObjectValue("status")
	if "expired" != status {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe checkout.expired status abnormal, ignored trade_no=%s status=%s", referenceId, status))
		return
	}

	if len(referenceId) == 0 {
		logger.LogWarn(ctx, "Stripe checkout.expired missing order id")
		return
	}
	...
	if errors.Is(err, model.ErrTopUpNotFound) {
		logger.LogWarn(ctx, fmt.Sprintf("Stripe top-up order does not exist, cannot mark as expired trade_no=%s", referenceId))
		return
	}
	if err != nil {
