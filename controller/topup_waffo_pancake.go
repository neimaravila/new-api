...
func SaveWaffoPancake(c *gin.Context) {
	...
	if err := service.SaveWaffoPancakeConfig(
		...
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "failed to save config"})
		return
	}
...
func CreateWaffoPancakeSubscriptionProduct(c *gin.Context) {
	...
	if merchantID == "" || privateKey == "" || storeID == "" {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Waffo Pancake is not fully configured, please complete gateway binding in payment settings first"})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "failed to create plan product"})
		return
	}
	c.JSON(http.StatusOK, gin.H{
		"message": "success",
...
func RequestWaffoPancakePay(c *gin.Context) {
	if !isWaffoPancakeTopUpEnabled() {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "Waffo Pancake config is incomplete"})
		return
	}
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook signature verification succeeded event_type=%s event_id=%s order_id=%s client_ip=%s", event.NormalizedEventType(), event.ID, event.Data.OrderID, c.ClientIP()))
