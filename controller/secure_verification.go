...
func UniversalVerify(c *gin.Context) {
	...
	if !ok {
		c.JSON(http.StatusUnauthorized, gin.H{"success": false, "message": "current authentication method does not support secure verification"})
		return
	}
	...
	if !isAllowedSecurityProofScope(request.Scope) {
		common.ApiError(c, errors.New("unsupported secure verification scope"))
		return
	}
	...
	if twoFA == nil || !twoFA.IsEnabled {
		common.ApiError(c, errors.New("2FA is not enabled for this user"))
		return
	}
	...
	model.RecordLog(identity.UserID, model.LogTypeSystem, "generic secure verification succeeded (method: 2FA)")
