...
func RequestWaffoPay(c *gin.Context) {
	...
	if req.PayMethodIndex != nil {
		...
		if idx < 0 || idx >= len(methods) {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo payment method index invalid user_id=%d pay_method_index=%d method_count=%d", id, idx, len(methods)))
		...
		if !valid {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo payment method invalid user_id=%d pay_method_type=%s pay_method_name=%q", id, req.PayMethodType, req.PayMethodName))
			c.JSON(http.StatusOK, gin.H{"message": "error", "data": "unsupported payment method"})
			return
		}
	}
	...
	if err != nil {
		...
		_ = topUp.Update()
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "payment config error"})
		return
	}
...
func WaffoWebhook(c *gin.Context) {
	...
	case core.EventPayment:
		...
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook signature verification and parse succeeded event_type=%s merchant_order_id=%s order_status=%s client_ip=%s", event.EventType, payload.Result.MerchantOrderID, payload.Result.OrderStatus, c.ClientIP()))
...
func handleWaffoPayment(c *gin.Context, wh *core.WebhookHandler, result *core.PaymentNotificationResult) {
	if result.OrderStatus != "PAY_SUCCESS" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo order status not successful, top-up ignored trade_no=%s order_status=%s client_ip=%s", result.MerchantOrderID, result.OrderStatus, c.ClientIP()))
