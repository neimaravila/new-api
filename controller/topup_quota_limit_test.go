...
func TestValidateTopUpQuotaReturnsMaximumAmount(t *testing.T) {
	...
	require.EqualError(t, err, "a single top-up amount cannot be greater than 4294")
}
...
func TestRequestAmountRejectsTopUpThatCannotBeSettled(t *testing.T) {
	...
	assert.JSONEq(t, `{"message":"error","data":"a single top-up amount cannot be greater than 4294"}`, recorder.Body.String())
}
