	...
	gopool.Go(func() {
		...
		if quotaTooLow {
			...
			if notifyType == dto.NotifyTypeBark {
				...
				content = "{{value}}, remaining quota: {{value}}, please recharge in time"
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else if notifyType == dto.NotifyTypeGotify {
				content = "{{value}}, current remaining quota is {{value}}, please recharge in time."
				values = []interface{}{prompt, logger.FormatQuota(relayInfo.UserQuota)}
			} else {
				// 默认内容格式，适用于Email和Webhook（支持HTML）
				content = "{{value}}, current remaining quota is {{value}}, to avoid service interruption please recharge in time.<br/>Recharge link: <a href='{{value}}'>{{value}}</a>"
