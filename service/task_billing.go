...
func RecalculateTaskQuota(ctx context.Context, task *model.Task, actualQuota int, reason string, clamps ...*common.QuotaClamp) {
	...
	if quotaDelta == 0 {
		logger.LogInfo(ctx, fmt.Sprintf("Task %s pre-consume accurate (%s, %s)",
			task.TaskID, logger.LogQuota(actualQuota), reason))
		return
	}

	logger.LogInfo(ctx, fmt.Sprintf("Task %s delta settlement: delta=%s (actual: %s, pre-consume: %s, %s)",
		task.TaskID,
	...
	if err := task.UpdateQuota(); err != nil {
		logger.LogError(ctx, fmt.Sprintf("Delta settlement failed to write back quota task %s: %s", task.TaskID, err.Error()))
	}
