...
func sweepTimedOutTasks(ctx context.Context) {
	...
	legacyReason := "Task timeout (legacy system task, no refund, please contact administrator)"
	now := time.Now().Unix()
	timedOutCount := 0
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	logger.LogInfo(ctx, fmt.Sprintf("Channel #%d incomplete tasks: %d", channelId, len(taskIds)))
	if ctx.Err() != nil {
		return ctx.Err()
	}
	if len(taskIds) == 0 {
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf("Channel #%d incomplete tasks: %d, successfully fetched task count: %s", channelId, len(taskIds), string(responseBody)))
		return err
	}
...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	...
	if bc := task.PrivateData.BillingContext; bc != nil && bc.PerCallBilling {
		logger.LogInfo(ctx, fmt.Sprintf("Task %s is billed per call, skipping delta settlement", task.TaskID))
		return
	}
