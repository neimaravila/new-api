...
autoDetect:
	...
	if host == "" {
		return nil, fmt.Errorf("Cannot determine Passkey Origin, please specify in system settings or Passkey settings. Current Host: '%s', ServerAddress: '%s'", r.Host, system_setting.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system_setting.PasskeySettings, origins []string) (string, error) {
	...
	if len(origins) == 0 {
		return "", errors.New("Passkey has no Origin configured, cannot derive RPID")
	}
