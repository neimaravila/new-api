...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	RecordTopupLog(topUp.UserId, fmt.Sprintf("online top-up succeeded, top-up amount: %v, paid amount: %f", logger.LogQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentProviderEpay)
	return false, nil
}
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	RecordTopupLog(topUp.UserId, fmt.Sprintf("online top-up succeeded, top-up amount: %v, payment amount: %d", logger.FormatQuota(quota), topUp.Amount), callerIp, topUp.PaymentMethod, PaymentMethodStripe)
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	RecordTopupLog(userId, fmt.Sprintf("admin manual completion succeeded, top-up amount: %v, payment amount: %f", logger.FormatQuota(quotaToAdd), payMoney), callerIp, paymentMethod, "admin")
	return nil
}
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	RecordTopupLog(topUp.UserId, fmt.Sprintf("Creem top-up succeeded, top-up quota: %v, payment amount: %.2f", quota, topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodCreem)
...
func RechargeWaffo(tradeNo string, callerIp string) (err error) {
	...
	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("Waffo top-up succeeded, top-up quota: %v, payment amount: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentMethodWaffo)
	}
...
func RechargeWaffoPancake(tradeNo string) (err error) {
	...
	if quotaToAdd > 0 {
		RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("Waffo Pancake top-up succeeded, top-up quota: %v, payment amount: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money))
	}
