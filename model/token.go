...
func validateLikePattern(input string) error {
	...
	if count > 2 {
		return errors.New("At most 2 % wildcards are allowed in the search pattern")
	}
...
func SearchUserTokens(userId int, keyword string, token string, offset int, limit int) (tokens []*Token, total int64, err error) {
	...
	if hasFuzzy {
		...
		if int(count) > maxTokens {
			return nil, 0, errors.New("Token count exceeds the limit, only exact search is allowed, please do not use % wildcards")
		}
	}
