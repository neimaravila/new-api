...
func (user *User) TransferAffQuotaToQuota(quota int) error {
	...
	if float64(quota) < common.QuotaPerUnit {
		return fmt.Errorf("minimum quota transfer is %s!", logger.LogQuota(common.QuotaFromFloat(common.QuotaPerUnit)))
	}
...
func (user *User) finishInsert(inviterId int) {
	...
	if inviterId != 0 && operation_setting.IsPaymentComplianceConfirmed() {
		...
		if common.QuotaForInviter > 0 {
			...
			RecordLog(inviterId, LogTypeSystem, fmt.Sprintf("inviting user bonus %s", logger.LogQuota(common.QuotaForInviter)))
